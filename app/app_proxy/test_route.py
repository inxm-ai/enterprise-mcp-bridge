...
class TestGetTargetUrl:
    ...
    def test_basic_path(self, mock_request, monkeypatch):
        ...
        monkeypatch.setattr("app.app_proxy.route.TARGET_SERVER_URL", TEST_TARGET_SERVER_URL)
    ...
    def test_special_characters_in_query(self, mock_request, monkeypatch):
        ...
        result = get_target_url(mock_request)
        assert result == f"{TEST_TARGET_SERVER_URL}/search?q=hello%20world&tag=foo%2Fbar"
...
class TestRewriteContentUrls:
    ...
    def test_javascript_url_rewriting(self):
        ...
        assert f"'{PROXY_PREFIX}/images/photo.jpg'" in result_str or f'"/images/photo.jpg"' in result_str
        assert TEST_TARGET_SERVER_URL not in result_str
...
class TestForwardToTarget:
    ...
    @pytest.mark.asyncio
    async def test_successful_get_request(self, mock_request, mock_httpx_response, monkeypatch):
        ...
        with patch.object(AsyncClient, 'request', new_callable=AsyncMock) as mock_client:
    ...
    @pytest.mark.asyncio
    async def test_successful_post_request(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_redirect_with_location_rewrite(self, mock_request, mock_httpx_response, monkeypatch):
        ...
        with patch.object(AsyncClient, 'request', new_callable=AsyncMock) as mock_client:
            ...
            assert TEST_TARGET_SERVER_URL not in dict(result.headers).get("location", "")
    
    @pytest.mark.asyncio
    async def test_cookie_path_rewriting(self, mock_request, mock_httpx_response, monkeypatch):
        """Test Set-Cookie header path rewriting."""
        monkeypatch.setattr("app.app_proxy.route.TARGET_SERVER_URL", TEST_TARGET_SERVER_URL)
    ...
    @pytest.mark.asyncio
    async def test_streaming_large_response(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_compressed_response(self, mock_request, mock_httpx_response, monkeypatch):
        """Test handling of gzip-compressed response."""
        monkeypatch.setattr("app.app_proxy.route.TARGET_SERVER_URL", TEST_TARGET_SERVER_URL)
    ...
    async def test_connection_error(self, mock_request, monkeypatch):
        ...
        with patch.object(AsyncClient, 'request', new_callable=AsyncMock) as mock_client:
            ...
            assert "502" in str(exc_info.value) or "gateway" in str(exc_info.value).lower()
    ...
    @pytest.mark.asyncio
    async def test_various_http_methods(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_requests(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_special_status_codes(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_preserves_custom_headers(self, mock_request, mock_httpx_response, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_removes_hop_by_hop_headers_from_response(self, mock_request, mock_httpx_response, monkeypatch):
