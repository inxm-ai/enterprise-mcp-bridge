...
def test_retrieve_token_success(monkeypatch, retriever):
    token_data = {"access_token": make_token(exp=(datetime.now().timestamp()+120)), "token_type": "Bearer", "expires_in": 120}
    monkeypatch.setattr(retriever, "_get_stored_provider_token", lambda t: token_data.copy())
...
def test_retrieve_token_refresh(monkeypatch, retriever):
    token_data = {"access_token": make_token(exp=(datetime.now().timestamp()-10)), "refresh_token": "refresh"}
    monkeypatch.setattr(retriever, "_get_stored_provider_token", lambda t: token_data.copy())
    monkeypatch.setattr(retriever, "_token_needs_refresh", lambda td: True)
    monkeypatch.setattr(retriever, "_refresh_provider_token", lambda td: {"access_token": "newtoken", "token_type": "Bearer", "expires_in": 3600})
    result = retriever.retrieve_token("dummy")
    assert result["success"] is True
...
def test_refresh_provider_token_success(monkeypatch, retriever):
    ...
    mock_response.json.return_value = {"access_token": "newtoken", "token_type": "Bearer", "expires_in": 3600}
...
def test_force_token_refresh_refresh(monkeypatch, retriever):
    monkeypatch.setattr(retriever, "_get_stored_provider_token", lambda t: {"refresh_token": "refresh"})
    monkeypatch.setattr(retriever, "_refresh_provider_token", lambda td: {"access_token": "newtoken"})
...
def test_force_token_refresh_broker(monkeypatch, retriever):
    ...
    monkeypatch.setattr(retriever, "_force_broker_refresh", lambda t: {"access_token": "broker_token"})
    result = retriever.force_token_refresh("tok")
    assert result["success"] is True
    assert result["access_token"] == "broker_token"

def test_force_token_refresh_exception(monkeypatch, retriever):
    monkeypatch.setattr(retriever, "_get_stored_provider_token", lambda t: (_ for _ in ()).throw(Exception("fail")))
    result = retriever.force_token_refresh("tok")
    assert result["success"] is False
