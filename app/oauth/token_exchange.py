...
class KeyCloakTokenRetriever(TokenRetriever):
    ...
    def retrieve_token(self, keycloak_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not provider_tokens:
                self.logger.info(f"Failed to retrieve {self.provider_alias} token from Keycloak")
                return {
                    "success": False,
            ...
            self.logger.info(f"Successfully retrieved {self.provider_alias} token from Keycloak")
            return {
                "success": True,
            ...
            return {
                "success": False,
                "error": f"Token retrieval failed"
            }
    ...
    def _get_stored_provider_token(self, keycloak_token: str) -> Dict[str, Any]:
        ...
        if response.status_code == 200:
            ...
            self.logger.warning("Unauthorized access to Keycloak broker endpoint - User may not have broker.read-token role")
            raise Exception("Unauthorized: User may not have broker.read-token role")
        else:
            self.logger.error(f"Failed to retrieve token from {url}: {response.status_code} - {response.text}")
    ...
    def force_token_refresh(self, keycloak_token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_tokens and current_tokens.get("refresh_token"):
                ...
                self.logger.info(f"[REFRESH]Forced refresh successful using refresh token: {str(new_token)}")
                return {
                    "success": True,
                ...
                self.logger.info(f"[BROKER]Forced refresh successful using broker: {str(new_token)}")
                return {
                    "success": True,
                    "access_token": new_token.get("access_token"),
                }
    ...
    def _force_broker_refresh(self, keycloak_token: str) -> Dict[str, Any]:
        ...
        if response.status_code == 200:
            ...
            self.logger.error(f"Failed to force broker refresh: {response.status_code} - {response.text}")
