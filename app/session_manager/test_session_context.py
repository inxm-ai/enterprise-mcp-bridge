...
async def test_sessionless_call_tool_injects_headers(monkeypatch):
    ...
    def dict_to_obj(d):
        filled = {k: d.get(k, None) for k in [
            "id",
            "name",
            "title",
            "description",
    ...
    tools_list = [
        dict_to_obj({
            "name": "toolA",
            "inputSchema": {"type": "object", "properties": {"userId": {"type": "string"}, "keep": {"type": "string"}}},
        })
    ]
    ...
    class _AsyncMCPContext:
        ...
        async def __aenter__(self):
            ...
            async def _call_tool(name, args):
                item = types.SimpleNamespace(text="ok", structuredContent={"args": args})
                return types.SimpleNamespace(content=[item], structuredContent={"args": args})
    ...
    monkeypatch.setattr(sc, "decorate_args_with_oauth_token", AsyncMock(side_effect=lambda *a, **k: a[2]))
    ...
    async with sc.mcp_session_context(sessions=None, x_inxm_mcp_session=None, access_token=None, group=None, incoming_headers=incoming_headers) as delegate:
