...
class MockTool:
    def __init__(self, name, title, description, inputSchema, outputSchema, annotations, meta):
        self.name = name
        self.title = title
...
def mock_tools():
    return MockTools([
        ...
        MockTool("special_tool", "Special Tool", "Special Description", {}, {}, {}, {}),
    ])
...
def test_works_without_filters(mock_tools):
    with patch("app.routes.INCLUDE_TOOLS", []), patch("app.routes.EXCLUDE_TOOLS", []):
        ...
        assert len(result) == 3
        assert all(tool["name"] in ["tool1", "tool2", "special_tool"] for tool in result)

def test_include_tools(mock_tools):
    with patch("app.routes.INCLUDE_TOOLS", ["tool*"]), patch("app.routes.EXCLUDE_TOOLS", []):
        result = map_tools(mock_tools)
        assert len(result) == 2
        assert all(tool["name"] in ["tool1", "tool2"] for tool in result)

def test_exclude_tools(mock_tools):
    with patch("app.routes.INCLUDE_TOOLS", []), patch("app.routes.EXCLUDE_TOOLS", ["tool*"]):
        result = map_tools(mock_tools)
        assert len(result) == 1
        assert result[0]["name"] == "special_tool"

def test_include_and_exclude_tools(mock_tools):
    with patch("app.routes.INCLUDE_TOOLS", ["*"]), patch("app.routes.EXCLUDE_TOOLS", ["special*"]):
        result = map_tools(mock_tools)
        assert len(result) == 2
        assert all(tool["name"] in ["tool1", "tool2"] for tool in result)
