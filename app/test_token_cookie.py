...
def test_cookie_auth(fastapi_app_cookie_auth):
    ...
    assert (
        r.status_code == 200
    ), f"Expected 200 with cookie, got {r.status_code}, {r.text}"
    ...
    r = session.post(
        f"{base_url}/session/start", headers={"X-Auth-Token": "my-secret-token"}
    )
    assert (
        r.status_code == 200
    ), f"Expected 200 (token is optional), got {r.status_code}"
