...
def mirror_request(path):
    ...
    if json_data:
        ...
        def generate_sse():
            if (
                json_data["messages"]
                and json_data["messages"][-1].get("role") == "tool"
            ):
                ...
                markdown_list = "\n".join(
                    [f"- {folder['displayName']}" for folder in folder_list]
                )
                    ...
                    "choices": [
                        {
                            "index": 0,
                            "delta": {
                                "content": "I'm not an llm, only a dummy implementation. I can only show you your inbox folders:\n"
                                + markdown_list
                            },
                            "finish_reason": "stop",
                        }
                ...
                tool_call = [
                    {
                        "index": 0,
                        "function": {"name": "list-mail-folders", "arguments": "{}"},
                    }
                ]
                event_data = {
                    "id": str(uuid.uuid4()),
                ...
                event_data = {
                    ...
                    "choices": [
                        {
                            "index": 0,
                            "delta": {"content": json.dumps(json_data)},
                            "finish_reason": "stop",
                        }
                    ],
                    "usage": {
